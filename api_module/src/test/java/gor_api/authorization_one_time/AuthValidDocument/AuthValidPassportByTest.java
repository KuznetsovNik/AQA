...
import static constant.DataConstants.LOGIN_VALID_PASSPORT_BY;
...
@TmsLink("https://vmmreg32.testrail.net/index.php?/cases/view/6386153")
public class AuthValidPassportByTest extends BaseAuthorization {

    @Test(enabled = false, suiteName = "user_service", groups = {"smoke", "regression"})
    @Description("Паспорт РБ. Валидные данные")
    void authWithPassportBy() {
        JsonResponse auth = authApiService.postPassport(headers,
                body(PASSPORT_BY.getCode(), LOGIN_VALID_PASSPORT_BY, PASSWORD, TYPE_DOCUMENT));
