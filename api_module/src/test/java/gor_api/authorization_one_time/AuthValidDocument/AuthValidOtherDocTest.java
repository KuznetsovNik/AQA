...
import static constant.DataConstants.LOGIN_VALID_OTHER_DOCUMENT;
...
@TmsLink("https://vmmreg32.testrail.net/index.php?/cases/view/6386158")
public class AuthValidOtherDocTest extends BaseAuthorization {

    @Test(suiteName = "user_service", groups = {"smoke", "regression"})
    @Description("Другой документ. Валидные данные")
    void authWithOtherDocument() {
        JsonResponse auth = authApiService.postPassport(headers,
                body(ANOTHER_DOC.getCode(), LOGIN_VALID_OTHER_DOCUMENT, VALID_PASSWORD, TYPE_DOCUMENT));
