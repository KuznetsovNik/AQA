...
public class InvalidTypeAuthotizationTest extends BaseAuthorization {

    @Test(suiteName = "user_service", groups = {"smoke", "regression"})
    @Description("Авторизация с типом - DOCUMENT_NUMBER")
    void typePassportTest() {
        JsonResponse auth = authApiService.postPhone(headers, body(RU.getCode(), LOGIN_VALID_RU, PASSWORD, TYPE_DOCUMENT));
