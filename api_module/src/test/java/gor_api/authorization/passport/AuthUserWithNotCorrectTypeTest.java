...
public class AuthUserWithNotCorrectTypeTest extends BaseAuthorization {

    @Test(suiteName = "user_service", groups = {"smoke", "regression"})
    @Description("Авторизация с невалидным типом")
    void authWithPassportTest() {
        JsonResponse auth = authApiService.postPassport(headers,
                body(PASSPORT_RU.getCode(), LOGIN_VALID_PASSPORT_RU, VALID_PASSWORD, TYPE_NOT_CORRECT));
